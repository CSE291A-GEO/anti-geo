...
global_cache = None
def call_gpt(user_prompt, system_prompt = COMMON_SYSTEM_PROMPT, model = 'gemini-2.5-flash', temperature = 0.0, num_completions = 1, regenerte_answer = False, pre_msgs = None):
    global global_cache
    ...
    for attempt in range(10):
        try:
            ...
            for _ in range(num_completions):
                # Gemini chat API with system + user messages
            ...
            if 'length' in str(e).lower() or 'context' in str(e).lower() or 'token' in str(e).lower():
                ...
                messages[-1]['content'] = messages[-1]['content'][:int(lup * 0.8)]  
            if attempt > 5:
